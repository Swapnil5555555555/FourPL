...
crown_hlg_kits_dp_query = """
    SELECT TRUNC(SYSDATE-0.1916) AS REPORT_DATE,CASE WHEN ORDER_TYPE='KTS' THEN 'CROWN' ELSE CASE WHEN CONSIGNMENT LIKE '%HLG%' THEN 'HLG' ELSE '' END END AS "KIT_BUILDER",
    ...
    FROM(
            ...
            WHERE OH.CLIENT_ID='VPNA' AND FROM_SITE_ID='MEM'
                    AND TRUNC(SHIP_BY_DATE)=TRUNC(SYSDATE-1.1916)
...
shipped_lines_query = """
    SELECT TRUNC(SYSDATE-0.1916) AS REPORT_DATE, CASE WHEN ORDER_TYPE='KTS' THEN 'CRW' ELSE 'HLG' END AS "3PL", SKU_ID, UPDATE_QTY AS SHIPPED_QTY, ORDER_TYPE, ORDER_ID, LINE_ID,
...
WHERE ITL.SITE_ID='MEM'
 AND TRUNC(DSTAMP)=TRUNC(SYSDATE-1.1916)
 AND CODE='Shipment'
...
open_kits_query = """
    SELECT 
        TRUNC(SYSDATE-0.1916) AS REPORT_DATE, 
        SHIP_BY_DATE, 
    ...
    AND TRUNC(SHIP_BY_DATE)<TRUNC(SYSDATE-0.1916)
...
inbound_receipts = """
    ...
    SELECT TRUNC(SYSDATE-0.1916) AS REPORT_DATE, 'CRW' AS "3PL",TRUNC(DSTAMP) AS DATE_, TAG_ID, ITL.SKU_ID, SUPPLIER_ID,  UPDATE_QTY AS QTY
...
open_asns = """
    ...
    WHERE PAH.SITE_ID='MEM'
           AND SUPPLIER_ID IN ('0008123','0753080039','0008121','8419','80039','32042','54197')
            AND TRUNC(DUE_DSTAMP)<TRUNC(SYSDATE-0.200)
...
THREE_PL_COST = """
                        SELECT TRUNC(SYSDATE-0.1916) AS REPORT_DATE,'LIP' AS "3PL", CODE,TRUNC(DSTAMP) AS TRANSACTION_DATE, TAG_ID, ITL.SKU_ID, SUPPLIER_ID, UPDATE_QTY AS QTY
                        ...
                        SELECT TRUNC(SYSDATE-0.1916) AS REPORT_DATE,'CRW' AS "3PL", CODE,TRUNC(DSTAMP) AS TRANSACTION_DATE, TAG_ID, ITL.SKU_ID, SUPPLIER_ID, UPDATE_QTY AS QTY
                        FROM DCSDBA.INVENTORY_TRANSACTION ITL
                        WHERE ITL.SITE_ID='MEM'
...
BACKTRACK_CRWN_COST = """
      SELECT TRUNC(DSTAMP+1.1916) AS REPORT_DATE,'CRW' AS "3PL", CODE,TRUNC(DSTAMP) AS TRANSACTION_DATE, TAG_ID, ITL.SKU_ID, SUPPLIER_ID, UPDATE_QTY AS QTY
                        FROM DCSDBA.INVENTORY_TRANSACTION ITL
                        WHERE ITL.SITE_ID='MEM'
                        AND FROM_LOC_ID LIKE 'CR%'
                        AND TO_LOC_ID NOT LIKE 'CR%'
